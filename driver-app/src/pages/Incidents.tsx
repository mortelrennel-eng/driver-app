  ...
  return (
    <IonPage>
      ...
      <IonContent fullscreen style={{ '--background': g.bg }}>
        ...
        <div style={{ minHeight: '100%', background: g.bg, padding: '20px' }}>
          
          <div style={{ marginBottom: '24px' }}>
            <h1 style={{ color: '#fff', fontSize: '28px', fontWeight: '900', margin: '0 0 8px' }}>My Incidents</h1>
            <p style={{ color: '#94a3b8', fontSize: '14px', margin: 0 }}>Review recorded behaviors and violations.</p>
          </div>
          ...
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '80px 20px', background: g.card, ...g.glass, border: g.border, borderRadius: '24px' }}>
              ...
              <div style={{ fontSize: '18px', fontWeight: '800', color: '#f8fafc', marginBottom: '8px' }}>No Incidents Found</div>
              <div style={{ color: '#64748b', fontSize: '14px' }}>You have a clean record! Keep driving safely.</div>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {incidents.map(incident => {
                ...
                return (
                  <div key={incident.id} style={{ padding: '20px', background: g.card, ...g.glass, border: g.border, borderRadius: '20px', boxShadow: '0 8px 32px rgba(0,0,0,0.3)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        ...
                        <div>
                          <div style={{ fontSize: '15px', fontWeight: '800', color: '#f8fafc' }}>{incident.incident_type}</div>
                    ...
                    <div style={{ background: 'rgba(0,0,0,0.2)', padding: '14px', borderRadius: '14px', marginBottom: '16px' }}>
                      ...
                      <div style={{ fontSize: '13px', color: '#cbd5e1', lineHeight: '1.5' }}>{incident.description}</div>
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                      <div style={{ padding: '12px', background: 'rgba(255,255,255,0.03)', borderRadius: '14px', border: '1px solid rgba(255,255,255,0.05)' }}>
                        <div style={{ fontSize: '9px', color: '#64748b', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '4px' }}>Unit Involved</div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                          <IonIcon icon={carOutline} style={{ fontSize: '14px', color: g.info }} />
                          <span style={{ fontSize: '13px', fontWeight: '700', color: '#f8fafc' }}>{incident.plate_number}</span>
                        </div>
                      </div>
                      <div style={{ padding: '12px', background: 'rgba(255,255,255,0.03)', borderRadius: '14px', border: '1px solid rgba(255,255,255,0.05)' }}>
                        <div style={{ fontSize: '9px', color: '#64748b', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '4px' }}>Charge Amount</div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    ...
                    {incident.total_charge_to_driver > 0 && (
                      <div style={{ marginTop: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '12px', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                        <div style={{ fontSize: '11px', color: '#94a3b8' }}>Status: <span style={{ fontWeight: '700', color: incident.charge_status === 'paid' ? '#22c55e' : g.warning }}>{incident.charge_status.toUpperCase()}</span></div>
                        <div style={{ fontSize: '11px', color: '#94a3b8' }}>Balance: <span style={{ fontWeight: '800', color: '#f8fafc' }}>₱{Number(incident.remaining_balance).toLocaleString()}</span></div>
                      </div>
                    )}
                  </div>
                );
