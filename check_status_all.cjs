...
async function runTest() {
    ...
    if (devRes.code === 0 && devRes.result) {
        console.log(`Current UTC Time: ${getTimestamp()}`);
        for (const d of devRes.result) {
            console.log(`[${d.deviceName}] status=${d.status} ACC=${d.accStatus} speed=${d.speed} gpsTime=${d.gpsTime} hbTime=${d.hbTime}`);
        }
    }
}
runTest();
