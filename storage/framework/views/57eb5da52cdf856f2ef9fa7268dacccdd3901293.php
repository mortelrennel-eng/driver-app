<?php $__env->startSection('page-heading', 'System History Logs'); ?>
<?php $__env->startSection('page-subheading', 'Detailed audit trail of all administrative and system actions.'); ?>
...
<div class="log-container">
    ...
    <div class="filter-card">
        <form action="<?php echo e(route('activity-logs.index')); ?>" method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-4">
            <div class="md:col-span-2">
                <label class="block text-[10px] uppercase font-bold text-gray-400 mb-1 px-1">Search Keywords</label>
                <div class="relative">
                    <i data-lucide="search" class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400"></i>
                    <input type="text" name="search" value="<?php echo e(request('search')); ?>" class="log-input pl-10" placeholder="Names, emails, actions, notes...">
                </div>
            </div>
            ...
            <div>
                ...
                <select name="type" class="log-input" onchange="this.form.submit()">
                    ...
                    <option value="auth" <?php echo e(request('type') === 'auth' ? 'selected' : ''); ?>>Login/Security</option>
                    <option value="admin" <?php echo e(request('type') === 'admin' ? 'selected' : ''); ?>>Admin Actions</option>
                    <option value="system" <?php echo e(request('type') === 'system' ? 'selected' : ''); ?>>System Logic</option>
                </select>
            </div>

            <div>
                <label class="block text-[10px] uppercase font-bold text-gray-400 mb-1 px-1">From Date</label>
                <input type="date" name="date_from" value="<?php echo e(request('date_from')); ?>" class="log-input">
            </div>

            <div>
                <label class="block text-[10px] uppercase font-bold text-gray-400 mb-1 px-1">To Date</label>
                <div class="flex gap-2">
                    <input type="date" name="date_to" value="<?php echo e(request('date_to')); ?>" class="log-input">
                    <button type="submit" class="bg-yellow-600 hover:bg-yellow-700 text-white p-2.5 rounded-xl transition-all">
                        <i data-lucide="filter" class="w-4 h-4"></i>
                    </button>
                    <a href="<?php echo e(route('activity-logs.index')); ?>" class="bg-gray-100 hover:bg-gray-200 text-gray-600 p-2.5 rounded-xl transition-all">
                ...
                <tr class="log-row">
                    <td class="pl-6">
                        <div class="flex flex-col">
                            <span class="font-bold text-slate-800"><?php echo e($log->created_at->format('M d, Y')); ?></span>
                            <span class="timestamp"><?php echo e($log->created_at->format('h:i:s A')); ?></span>
                            <span class="text-[10px] text-slate-400 uppercase tracking-tighter mt-1"><?php echo e($log->created_at->diffForHumans()); ?></span>
                        </div>
                    </td>
                    <td>
                        <div class="flex items-center gap-3">
                            <div class="user-avatar shadow-sm">
                                <?php echo e(strtoupper(substr($log->user_name ?? 'U', 0, 1))); ?>

                            </div>
                            <div>
                                <div class="font-semibold text-slate-900 leading-tight"><?php echo e($log->user_name ?? 'System'); ?></div>
                                <div class="text-xs text-slate-500"><?php echo e($log->user_email ?? 'automated@task.system'); ?></div>
                                <div class="mt-1">
                                    <span class="text-[10px] px-1.5 py-0.5 rounded bg-slate-100 text-slate-600 font-bold border border-slate-200">
                                        <?php echo e(strtoupper($log->user_role ?? 'System')); ?>
                    ...
                    <td>
                        ...
                        ?>
                        <span class="action-badge <?php echo e($class); ?>">
                            <i data-lucide="<?php echo e($icon); ?>" class="w-3 h-3"></i>
                    ...
                    <td class="pr-6">
                        <div class="flex flex-col items-end">
                            ...
                            <div class="text-[9px] text-slate-400 mt-1 max-w-[120px] truncate" title="<?php echo e($log->user_agent); ?>">
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/u747826271/domains/eurotaxisystem.site/public_html/resources/views/activity-log/index.blade.php ENDPATH**/ ?>