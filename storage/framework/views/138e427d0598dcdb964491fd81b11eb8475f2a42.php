<?php $__env->startSection('title', 'Driver Performance - Euro System'); ?>
<?php $__env->startSection('page-heading', 'Driver Performance & Violations'); ?>
<?php $__env->startSection('page-subheading', 'Incidents • Incentives • Driver Profiles — All in one place'); ?>
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    ...
    <div class="stat-card-premium relative overflow-hidden bg-gradient-to-br from-red-600 to-rose-700 rounded-2xl p-4 text-white shadow-lg shadow-red-100 group">
        ...
        <div class="relative z-10 flex flex-col items-center text-center">
            <p class="text-3xl font-black tracking-tighter leading-none"><?php echo e($stats['violations_today'] ?? 0); ?></p>
    ...
    <div class="stat-card-premium relative overflow-hidden bg-gradient-to-br from-teal-500 to-emerald-600 rounded-2xl p-4 text-white shadow-lg shadow-teal-100 group">
        ...
        <div class="relative z-10 flex flex-col items-center text-center">
            <p class="text-3xl font-black tracking-tighter leading-none"><?php echo e($stats['total_violators'] ?? 0); ?></p>
    ...
    <div class="stat-card-premium relative overflow-hidden bg-gradient-to-br from-purple-600 to-indigo-700 rounded-2xl p-4 text-white shadow-lg shadow-purple-100 group">
        ...
        <div class="relative z-10 flex flex-col items-center text-center">
            <p class="text-xl font-black tracking-tighter leading-none">₱<?php echo e(number_format($stats['total_charges'] ?? 0, 0)); ?></p>
    ...
    <div class="stat-card-premium relative overflow-hidden bg-gradient-to-br from-yellow-500 to-orange-500 rounded-2xl p-4 text-white shadow-lg shadow-yellow-100 group">
        ...
        <div class="relative z-10 flex flex-col items-center text-center">
            <p class="text-3xl font-black tracking-tighter leading-none"><?php echo e(count($incentive_summary['eligible'] ?? [])); ?></p>
...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-3 mb-5 flex flex-wrap gap-2">
    <button onclick="switchTab('incidents')" id="tab-btn-incidents"
        class="tab-btn <?php echo e(($tab ?? 'incidents') === 'incidents' ? 'active' : ''); ?>">
        <i data-lucide="list" class="w-3.5 h-3.5 inline mr-1"></i> Incident Log
    </button>
    <button onclick="switchTab('incentives')" id="tab-btn-incentives"
        class="tab-btn <?php echo e(($tab ?? '') === 'incentives' ? 'active' : ''); ?>">
        <i data-lucide="trophy" class="w-3.5 h-3.5 inline mr-1"></i>
        Incentive Dashboard
        <?php if(count($incentive_summary['eligible'] ?? []) > 0): ?>
            <span class="ml-1 px-1.5 py-0.5 bg-green-500 text-white text-[9px] rounded-full"><?php echo e(count($incentive_summary['eligible'])); ?></span>
        <?php endif; ?>
    </button>
    <button onclick="switchTab('profiles')" id="tab-btn-profiles"
        class="tab-btn <?php echo e(($tab ?? '') === 'profiles' ? 'active' : ''); ?>">
    ...
    <button onclick="switchTab('accidents')" id="tab-btn-accidents"
        class="tab-btn <?php echo e(($tab ?? '') === 'accidents' ? 'active' : ''); ?>">
        <i data-lucide="triangle-alert" class="w-3.5 h-3.5 inline mr-1 text-red-500"></i> Accident Reports
        <?php if(isset($accident_reports) && $accident_reports->where('status', 'pending')->count() > 0): ?>
            <span class="ml-1 px-1.5 py-0.5 bg-red-500 text-white text-[9px] rounded-full animate-pulse"><?php echo e($accident_reports->where('status', 'pending')->count()); ?></span>
        <?php endif; ?>
    </button>
...
<div id="tab-incidents" class="tab-content <?php echo e(($tab ?? 'incidents') === 'incidents' ? '' : 'hidden'); ?>">

    
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 mb-4">
        <form method="GET" action="<?php echo e(route('driver-behavior.index')); ?>" class="flex flex-wrap gap-3 items-end">
            ...
            <div class="flex-1 min-w-[180px]">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="search" value="<?php echo e($search); ?>" placeholder="Driver, unit, description..."
            ...
            <div class="w-40">
                ...
                <select name="type" onchange="this.form.submit()" class="w-full px-3 py-2 bg-gray-50 border border-gray-100 rounded-xl text-sm font-medium focus:ring-2 focus:ring-yellow-500 focus:outline-none">
                    ...
                    <?php $__currentLoopData = $classifications; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($c->name); ?>" <?php echo e($type_filter === $c->name ? 'selected' : ''); ?>><?php echo e($c->name); ?></option>
            ...
            <div class="w-36">
                ...
                <input type="date" name="date_from" value="<?php echo e($date_from); ?>" onchange="this.form.submit()"
            ...
            <div class="w-36">
                ...
                <input type="date" name="date_to" value="<?php echo e($date_to); ?>" onchange="this.form.submit()"
                    class="w-full px-3 py-2 bg-gray-50 border border-gray-100 rounded-xl text-sm font-medium focus:ring-2 focus:ring-yellow-500 focus:outline-none">
            </div>
        </form>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors group">
                        <td class="px-5 py-3.5 whitespace-nowrap">
                            <div class="text-xs font-bold text-gray-800"><?php echo e(\Carbon\Carbon::parse($inc->timestamp)->timezone('Asia/Manila')->format('M d, Y')); ?></div>
                            <div class="text-[10px] text-gray-400 font-medium"><?php echo e(\Carbon\Carbon::parse($inc->timestamp)->timezone('Asia/Manila')->format('h:i A')); ?></div>
                        </td>
                        <td class="px-5 py-3.5 whitespace-nowrap">
                            <div class="text-xs font-bold text-gray-800"><?php echo e($inc->driver_name ?? '—'); ?></div>
                        </td>
                        <td class="px-5 py-3.5 whitespace-nowrap">
                            <span class="text-xs font-black text-blue-600 uppercase"><?php echo e($inc->plate_number ?? '—'); ?></span>
                        </td>
                        <td class="px-5 py-3.5 max-w-[450px]">
                            
                            <div class="flex flex-wrap gap-1.5 mb-2">
                                
                                <?php if($inc->is_driver_fault): ?>
                                ...
                                <?php if($inc->total_charge_to_driver > 0): ?>
                                    <span class="px-2 py-0.5 bg-purple-600 text-white text-[9px] font-black uppercase tracking-widest rounded-full shadow-sm shadow-purple-100">
                                        Amount: ₱<?php echo e(number_format($inc->total_charge_to_driver, 2)); ?>
                            ...
                            <?php if($inc->cause_of_incident): ?>
                                <div class="mb-1.5">
                                    <span class="text-[9px] font-black text-orange-600 bg-orange-50 px-1.5 py-0.5 rounded-full border border-orange-100 uppercase tracking-widest">Cause: <?php echo e($inc->cause_of_incident); ?></span>
                                </div>
                            <?php endif; ?>

                            <p class="text-xs text-gray-800 font-medium leading-relaxed"><?php echo e($inc->description); ?></p>
                        </td>

                        <td class="px-5 py-3.5 whitespace-nowrap">
                            <?php if(in_array($inc->severity, ['high','critical']) || $inc->is_driver_fault): ?>
                        ...
                        <td class="px-5 py-3.5 whitespace-nowrap text-right">
                            <div class="flex justify-end items-center gap-2">
                                ...
                                <button type="button" 
                                    onclick="IncidentManager.openEdit(<?php echo e($inc->id); ?>)"
                                ...
                                <button type="button" 
                                    onclick="IncidentManager.archive(<?php echo e($inc->id); ?>)"
        ...
        <div class="px-5 py-4 border-t border-gray-50 flex items-center justify-between">
            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between font-bold text-[10px] text-gray-400 uppercase tracking-widest">
                <div>
                    <p>Showing <span class="text-gray-900"><?php echo e(min($pagination['total_items'], ($pagination['page'] - 1) * 10 + 1)); ?></span> to <span class="text-gray-900"><?php echo e(min($pagination['total_items'], $pagination['page'] * 10)); ?></span> of <span class="text-gray-900"><?php echo e($pagination['total_items']); ?></span> incidents</p>
                </div>
                <div>
                    <nav class="relative z-0 inline-flex rounded-xl shadow-sm -space-x-px" aria-label="Pagination">
                        <?php if($pagination['has_prev']): ?>
                            <a href="<?php echo e(request()->fullUrlWithQuery(['page' => $pagination['prev_page']])); ?>" class="relative inline-flex items-center px-2 py-2 rounded-l-xl border border-gray-200 bg-white text-gray-400 hover:bg-gray-50">
                        ...
                        <?php for($i = $start; $i <= $end; $i++): ?>
                            <a href="<?php echo e(request()->fullUrlWithQuery(['page' => $i])); ?>" 
                               class="relative inline-flex items-center px-4 py-2 border text-[11px] font-black <?php echo e($i === $pagination['page'] ? 'z-10 bg-yellow-500 border-yellow-500 text-white shadow-lg shadow-yellow-500/20' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50'); ?>">
                                <?php echo e($i); ?>
                        ...
                        <?php if($pagination['has_next']): ?>
                            <a href="<?php echo e(request()->fullUrlWithQuery(['page' => $pagination['next_page']])); ?>" class="relative inline-flex items-center px-2 py-2 rounded-r-xl border border-gray-200 bg-white text-gray-400 hover:bg-gray-50">
            ...
            <div class="flex-1 flex justify-between sm:hidden">
                <?php if($pagination['has_prev']): ?>
                    <a href="<?php echo e(request()->fullUrlWithQuery(['page' => $pagination['prev_page']])); ?>" class="relative inline-flex items-center px-4 py-2 border border-gray-200 text-xs font-bold rounded-xl text-gray-700 bg-white hover:bg-gray-50">Previous</a>
                <?php endif; ?>
                <?php if($pagination['has_next']): ?>
                    <a href="<?php echo e(request()->fullUrlWithQuery(['page' => $pagination['next_page']])); ?>" class="ml-3 relative inline-flex items-center px-4 py-2 border border-gray-200 text-xs font-bold rounded-xl text-gray-700 bg-white hover:bg-gray-50">Next</a>
                <?php endif; ?>
            </div>
        </div>
        <?php endif; ?>
...
<div id="tab-incentives" class="tab-content <?php echo e(($tab ?? '') === 'incentives' ? '' : 'hidden'); ?>">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
        <div class="bg-gradient-to-br from-green-500 to-emerald-600 rounded-2xl p-5 text-white shadow-lg">
            ...
            <p class="text-3xl font-black"><?php echo e(count($incentive_summary['eligible'] ?? [])); ?></p>
        ...
        <div class="bg-gradient-to-br from-red-500 to-rose-600 rounded-2xl p-5 text-white shadow-lg">
            ...
            <p class="text-3xl font-black"><?php echo e(count($incentive_summary['ineligible'] ?? [])); ?></p>
        ...
        <div class="bg-gradient-to-br from-yellow-500 to-amber-600 rounded-2xl p-5 text-white shadow-lg">
            ...
            ?>
            <p class="text-xl font-black"><?php echo e($targetDate->format('M d, Y')); ?></p>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-5">
        <div class="px-5 py-4 border-b bg-green-50/50 flex items-center gap-2">
            ...
            <h3 class="font-black text-sm text-gray-800 uppercase tracking-widest">Eligible Drivers (<?php echo e(count($incentive_summary['eligible'] ?? [])); ?>)</h3>
        </div>
        <table class="min-w-full divide-y divide-gray-50">
            <thead class="bg-gray-50/80">
                <tr>
            ...
            <tbody class="divide-y divide-gray-50">
                <?php $__empty_1 = true; $__currentLoopData = $incentive_summary['eligible']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $d): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <tr class="hover:bg-green-50/30 transition-colors">
                    <td class="px-5 py-3.5"><span class="text-xs font-black text-gray-800"><?php echo e($d['name']); ?></span></td>
                    <td class="px-5 py-3.5"><span class="text-xs font-black text-blue-600 uppercase"><?php echo e($d['unit'] ?? '—'); ?></span></td>
                    <td class="px-5 py-3.5">
                        <span class="text-[9px] font-black px-2 py-0.5 rounded-full <?php echo e($d['driver_type'] === 'Dual Driver' ? 'bg-purple-100 text-purple-700 border border-purple-200' : 'bg-blue-100 text-blue-700 border border-blue-200'); ?>"><?php echo e($d['driver_type']); ?></span>
                    </td>
                    <td class="px-5 py-3.5">
                        <div class="flex items-center gap-2">
                            <div class="w-20 h-1.5 bg-gray-100 rounded-full"><div class="h-1.5 bg-green-500 rounded-full" style="width:<?php echo e(min(100, ($d['valid_days']/20)*100)); ?>%"></div></div>
                            <span class="text-xs font-black text-green-600"><?php echo e($d['valid_days']); ?>/20</span>
                        </div>
                    </td>
                    <td class="px-5 py-3.5"><span class="text-xs font-black <?php echo e($d['violations'] > 0 ? 'text-red-600' : 'text-green-600'); ?>"><?php echo e($d['violations']); ?></span></td>
                    <td class="px-5 py-3.5"><span class="text-xs font-medium text-gray-600"><?php echo e($d['next_payout']); ?></span></td>
                    <td class="px-5 py-3.5">
                        <form method="POST" action="<?php echo e(route('driver-behavior.release-incentive')); ?>" onsubmit="return confirm('Release incentive for <?php echo e(addslashes($d['name'])); ?>? This will reset their counter.')">
                            <?php echo csrf_field(); ?>
                            <input type="hidden" name="driver_id" value="<?php echo e($d['driver_id']); ?>">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-5 py-4 border-b bg-red-50/50 flex items-center gap-2">
            ...
            <h3 class="font-black text-sm text-gray-800 uppercase tracking-widest">Disqualified / Pending (<?php echo e(count($incentive_summary['ineligible'] ?? [])); ?>)</h3>
        </div>
        <table class="min-w-full divide-y divide-gray-50">
            <thead class="bg-gray-50/80">
                <tr>
            ...
            <tbody class="divide-y divide-gray-50">
                <?php $__empty_1 = true; $__currentLoopData = $incentive_summary['ineligible']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $d): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <?php $reason = $d['violations'] > 0 ? 'Has Violations' : 'Insufficient Days'; ?>
                <tr class="hover:bg-red-50/20 transition-colors">
                    <td class="px-5 py-3.5"><span class="text-xs font-bold text-gray-700"><?php echo e($d['name']); ?></span></td>
                    <td class="px-5 py-3.5"><span class="text-xs font-black text-blue-600 uppercase"><?php echo e($d['unit'] ?? '—'); ?></span></td>
                    <td class="px-5 py-3.5">
                        <span class="text-[9px] font-black px-2 py-0.5 rounded-full <?php echo e($d['driver_type'] === 'Dual Driver' ? 'bg-purple-100 text-purple-700 border border-purple-200' : 'bg-blue-100 text-blue-700 border border-blue-200'); ?>"><?php echo e($d['driver_type']); ?></span>
                    </td>
                    <td class="px-5 py-3.5">
                        <div class="flex items-center gap-2">
                            <div class="w-20 h-1.5 bg-gray-100 rounded-full"><div class="h-1.5 bg-red-400 rounded-full" style="width:<?php echo e(min(100, ($d['valid_days']/20)*100)); ?>%"></div></div>
                            <span class="text-xs font-black text-red-500"><?php echo e($d['valid_days']); ?>/20</span>
                        </div>
                    </td>
                    <td class="px-5 py-3.5"><span class="text-xs font-black <?php echo e($d['violations'] > 0 ? 'text-red-600' : 'text-gray-400'); ?>"><?php echo e($d['violations']); ?></span></td>
                    <td class="px-5 py-3.5">
                        <span class="text-[9px] font-black px-2 py-0.5 rounded-full bg-red-100 text-red-700 border border-red-200"><?php echo e($reason); ?></span>
                    </td>
                </tr>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
...
<div id="tab-accidents" class="tab-content <?php echo e(($tab ?? '') === 'accidents' ? '' : 'hidden'); ?>">
        ...
        <div class="overflow-x-auto min-h-[400px]">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-100/60 bg-white" id="accidentTableBody">
                    <?php $__empty_1 = true; $__currentLoopData = $accident_reports ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $report): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr class="hover:bg-red-50/30 transition-colors <?php echo e($report->status === 'pending' ? 'bg-red-50' : ''); ?>" id="sos-row-<?php echo e($report->id); ?>">
                        <td class="px-5 py-4">
                            <p class="text-xs font-bold text-gray-900"><?php echo e(\Carbon\Carbon::parse($report->created_at)->format('M d, Y')); ?></p>
                            <p class="text-[10px] text-gray-500 font-medium"><?php echo e(\Carbon\Carbon::parse($report->created_at)->format('h:i A')); ?></p>
                        </td>
                        <td class="px-5 py-4">
                            <p class="text-sm font-bold text-gray-900 truncate"><?php echo e($report->driver->first_name ?? ''); ?> <?php echo e($report->driver->last_name ?? ''); ?></p>
                            <p class="text-[10px] font-black text-gray-400 tracking-wider uppercase mt-0.5"><?php echo e($report->unit->plate_number ?? 'N/A'); ?></p>
                        </td>
                        <td class="px-5 py-4">
                            <div class="text-xs text-gray-600 line-clamp-3 whitespace-pre-wrap"><?php echo e($report->notes ?? 'No additional notes provided.'); ?></div>
                            <?php if($report->photo_path): ?>
                                <a href="<?php echo e(asset($report->photo_path)); ?>" target="_blank" class="mt-2 inline-flex items-center gap-1 text-[10px] font-bold text-blue-600 hover:text-blue-800 bg-blue-50 px-2 py-1 rounded-md transition-colors">
                        ...
                        <td class="px-5 py-4">
                            <?php if($report->latitude && $report->longitude): ?>
                                <div class="flex flex-col gap-1">
                                    <span class="text-xs font-medium text-gray-800 address-resolver flex items-center gap-1" data-lat="<?php echo e($report->latitude); ?>" data-lng="<?php echo e($report->longitude); ?>">
                                        <i data-lucide="loader-2" class="w-3 h-3 animate-spin text-blue-500"></i> Resolving address...
                                    </span>
                                    <a href="https://maps.google.com/?q=<?php echo e($report->latitude); ?>,<?php echo e($report->longitude); ?>" target="_blank" class="flex items-center gap-1 text-[10px] font-bold text-blue-600 hover:text-blue-800 uppercase tracking-widest transition-colors group w-max">
                        ...
                        <td class="px-5 py-4">
                            ...
                            <?php else: ?>
                                ...
                                <?php if($report->acknowledged_by): ?>
                                    <p class="text-[9px] text-gray-400 mt-1">Ack. by <?php echo e(\App\Models\User::find($report->acknowledged_by)?->first_name ?? 'Admin'); ?></p>
                                <?php endif; ?>
                            <?php endif; ?>
                        </td>
                        <td class="px-5 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <button onclick="viewSosDetails(<?php echo e($report->id); ?>)" class="p-2 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-lg transition-colors" title="View Details">
                                    <i data-lucide="eye" class="w-4 h-4"></i>
                                </button>
                                <button onclick="archiveSos(<?php echo e($report->id); ?>)" class="p-2 bg-gray-50 text-gray-500 hover:bg-gray-200 hover:text-gray-800 rounded-lg transition-colors" title="Archive">
...
<div id="tab-profiles" class="tab-content <?php echo e(($tab ?? '') === 'profiles' ? '' : 'hidden'); ?>">
    <div class="mb-4">
    ...
    <div id="profileGrid" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        <?php $__currentLoopData = $driver_profiles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $profile): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <?php
            $inc = $profile['incentive'];
            $eligible = $inc['eligible'];
        ?>
        <div class="profile-card bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-all cursor-pointer" data-name="<?php echo e(strtolower($profile['name'])); ?>" onclick="openDriverDetails(<?php echo e($profile['id']); ?>)">
            
            <div class="p-5 border-b border-gray-50 flex items-center gap-3 <?php echo e($eligible ? 'bg-gradient-to-r from-green-50 to-emerald-50' : 'bg-gray-50/50'); ?>">
                <div class="w-11 h-11 rounded-xl <?php echo e($eligible ? 'bg-green-500' : 'bg-gray-300'); ?> flex items-center justify-center text-white font-black text-lg shadow-sm flex-shrink-0">
                    <?php echo e(strtoupper(substr($profile['name'], 0, 1))); ?>

                </div>
                <div class="flex-1 min-w-0">
                    <p class="font-black text-sm text-gray-800 truncate"><?php echo e($profile['name']); ?></p>
                    <p class="text-[10px] font-bold text-blue-600 uppercase"><?php echo e($profile['unit'] ?? 'No Unit Assigned'); ?></p>
                </div>
                <div>
                    <?php if($eligible): ?>
            ...
            <div class="grid grid-cols-3 divide-x divide-gray-50 border-b border-gray-50">
                <div class="p-3 text-center">
                    <p class="text-lg font-black text-gray-800"><?php echo e($profile['incidents']); ?></p>
                    <p class="text-[9px] font-black text-gray-400 uppercase tracking-wider">Incidents</p>
                </div>
                <div class="p-3 text-center">
                    <p class="text-lg font-black text-gray-800"><?php echo e($profile['boundaries']); ?></p>
                    <p class="text-[9px] font-black text-gray-400 uppercase tracking-wider">Shifts</p>
                </div>
                <div class="p-3 text-center">
                    <p class="text-lg font-black <?php echo e($profile['charges'] > 0 ? 'text-red-600' : 'text-green-600'); ?>">₱<?php echo e(number_format($profile['charges'], 0)); ?></p>
            ...
            <div class="p-4">
                <div class="flex justify-between items-center mb-1.5">
                    <span class="text-[10px] font-black text-gray-500 uppercase tracking-widest"><?php echo e($inc['driver_type']); ?></span>
                    <span class="text-[10px] font-bold text-gray-500"><?php echo e($inc['valid_days']); ?>/<?php echo e($inc['required_days']); ?> valid days</span>
                </div>
                <div class="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
                    <div class="h-2 rounded-full transition-all <?php echo e($eligible ? 'bg-green-500' : 'bg-yellow-400'); ?>"
                        style="width: <?php echo e(min(100, ($inc['valid_days'] / $inc['required_days']) * 100)); ?>%"></div>
                </div>
                <div class="flex justify-between items-center mt-2">
                    <span class="text-[10px] text-gray-400"><?php echo e($inc['violations']); ?> violation(s)</span>
                    <span class="text-[10px] font-bold text-gray-500">Next: <?php echo e($inc['next_payout_date']); ?></span>
                </div>
                <?php if($profile['total_debt'] > 0): ?>
                <div class="mt-2 flex items-center gap-1.5 text-[10px] font-bold text-red-600 bg-red-50 px-2 py-1 rounded-lg border border-red-100">
                    <i data-lucide="alert-circle" class="w-3 h-3 text-red-500"></i> Pending Debt: ₱<?php echo e(number_format($profile['total_debt'], 2)); ?>

                </div>
                <?php endif; ?>
                <?php if($profile['shortages'] > 0): ?>
                <div class="mt-2 flex items-center gap-1.5 text-[10px] font-bold text-orange-600">
                    <i data-lucide="trending-down" class="w-3 h-3"></i> Total Shortage: ₱<?php echo e(number_format($profile['shortages'], 2)); ?>
...
<div id="incidentModal" class="fixed inset-0 bg-black/60 backdrop-blur-md hidden z-[100] flex items-center justify-center p-4">
    <div class="w-full max-w-2xl bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in duration-300">
        ...
        <form method="POST" action="<?php echo e(route('driver-behavior.store')); ?>" id="incidentForm" class="flex flex-col flex-1 overflow-hidden">
            <?php echo csrf_field(); ?>
                ...
                <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-5">
                            ...
                            <div id="unitSearchDropdown" class="search-dropdown hidden">
                                <?php $__currentLoopData = $units; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $u): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <div class="search-option unit-search-option" 
                                        ...
                                        data-name="<?php echo e($u->plate_number); ?>"
                                        data-driver-id="<?php echo e($u->driver_id); ?>"
                                        data-secondary-driver-id="<?php echo e($u->secondary_driver_id); ?>">
                                        <div class="font-black text-xs text-gray-900"><?php echo e($u->plate_number); ?></div>
                                    </div>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </div>
                        </div>
                        <div class="relative">
                            ...
                            <div id="driverSearchDropdown" class="search-dropdown hidden">
                                <?php $__currentLoopData = $drivers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $d): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <div class="search-option driver-search-option" data-id="<?php echo e($d->id); ?>" data-name="<?php echo e($d->full_name); ?>" data-contact="<?php echo e($d->contact_number ?? ''); ?>">
                                        <div class="flex justify-between items-start">
                                            <div class="font-black text-xs text-gray-900"><?php echo e($d->full_name); ?></div>
                                        ...
                                        <div class="text-[9px] text-gray-400 font-black uppercase tracking-tighter mt-1"><?php echo e($d->current_plate ?? 'Floating / Unassigned'); ?></div>
                                    </div>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </div>
                        </div>
                    ...
                    <div class="grid grid-cols-2 gap-5">
                            ...
                            <select name="incident_type" required id="incidentTypeSelect" onchange="handleTypeChange(this.value)"
                                ...
                                <?php $__currentLoopData = $classifications; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($c->name); ?>"
                                        data-mode="<?php echo e($c->behavior_mode ?? 'narrative'); ?>"
                                        data-sub-options='<?php echo e(json_encode($c->sub_options ?? \App\Models\IncidentClassification::getDefaultSubOptions($c->behavior_mode ?? 'narrative'))); ?>'
                                        data-auto-ban="<?php echo e($c->auto_ban_trigger ? '1' : '0'); ?>"
                                        data-ban-value="<?php echo e($c->ban_trigger_value ?? ''); ?>">
                                        <?php echo e($c->name); ?>
                        ...
                        <div class="col-span-2 sm:col-span-1">
                            ...
                            <div class="relative">
                                <input type="date" name="incident_date" value="<?php echo e(date('Y-m-d')); ?>" required
                                    class="w-full px-4 py-3.5 bg-gray-50 border border-gray-100 rounded-2xl text-sm font-bold focus:ring-4 focus:ring-yellow-500/10 focus:border-yellow-500 focus:outline-none transition-all">
                    ...
                    <div class="bg-white p-6 rounded-3xl border border-purple-100 shadow-sm space-y-4">
                        ...
                        <div class="relative">
                            ...
                            <div id="incidentPartDropdown" class="search-dropdown hidden max-h-60 overflow-y-auto">
                                <?php $__currentLoopData = $spare_parts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <?php $isAvailable = ($p->stock_quantity ?? 0) > 0; ?>
                                    <div class="search-option part-search-option group <?php echo e(!$isAvailable ? 'opacity-60 cursor-not-allowed bg-gray-50' : ''); ?>" 
                                        ...
                                        <div class="flex justify-between items-start w-full">
                                            <div>
                                                <div class="font-black text-xs <?php echo e($isAvailable ? 'text-gray-900' : 'text-gray-400'); ?>"><?php echo e($p->name); ?></div>
                                                <div class="flex items-center gap-2 mt-1">
                                                    <span class="text-[9px] font-black px-1.5 py-0.5 rounded <?php echo e($isAvailable ? 'bg-green-100 text-green-700' : 'bg-red-500 text-white shadow-sm'); ?>">
                                                        <?php echo e($isAvailable ? 'STOCK: ' . $p->stock_quantity : 'UNAVAILABLE'); ?>

                                                    </span>
                                                    <span class="text-[9px] text-gray-400 font-bold uppercase tracking-tighter italic">Supplier: <?php echo e($p->supplier ?? 'Unknown'); ?></span>
                                                </div>
                                            </div>
                                            <div class="text-[10px] font-black text-purple-600">₱<?php echo e(number_format($p->price, 2)); ?></div>
                                        </div>
                                    </div>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </div>
            ...
            <div class="p-8 border-r border-gray-100 flex flex-col max-h-[70vh]">
                ...
                <div id="clsList-active" class="cls-list-content space-y-3 overflow-y-auto custom-scroll pr-2 flex-1">
                    ...
                    <div class="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100 hover:border-yellow-200 transition-all group">
                        <div>
                            <div class="text-sm font-black text-gray-900"><?php echo e($c->name); ?></div>
                            <div class="text-[10px] font-bold text-gray-400 uppercase tracking-tighter">Default: <span class="text-yellow-600"><?php echo e($c->default_severity); ?></span></div>
                        </div>
                        <div class="flex gap-2 opacity-0 group-hover:opacity-100 transition-all relative z-10">
                            <button type="button" onclick="editClassification(<?php echo e($c->id); ?>, '<?php echo e(addslashes($c->name)); ?>', '<?php echo e($c->default_severity); ?>')" class="p-2 bg-white text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl border border-gray-100 shadow-sm transition-all cursor-pointer relative z-10">
                                <i data-lucide="edit-3" class="w-4 h-4"></i>
                            </button>
                            <button type="button" onclick="archiveClassification(<?php echo e($c->id); ?>)" class="p-2 bg-white text-gray-400 hover:text-orange-500 hover:bg-orange-50 rounded-xl border border-gray-100 shadow-sm transition-all cursor-pointer relative z-10">
                                <i data-lucide="archive" class="w-4 h-4"></i>
                            </button>
                        </div>
                    </div>
                ...
                <div id="clsList-archived" class="cls-list-content hidden space-y-3 overflow-y-auto custom-scroll pr-2 flex-1">
                    <?php $__empty_1 = true; $__currentLoopData = $archivedClassifications; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ac): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <div class="flex items-center justify-between p-4 bg-gray-100/50 rounded-2xl border border-gray-200 opacity-60">
                        <div class="text-sm font-black text-gray-500 line-through"><?php echo e($ac->name); ?></div>
                        <div class="flex gap-2 relative z-10">
                            <button type="button" onclick="restoreClassification(<?php echo e($ac->id); ?>)" class="p-2 bg-white text-gray-400 hover:text-green-600 hover:bg-green-50 rounded-xl border border-gray-100 transition-all cursor-pointer relative z-10">
                                <i data-lucide="refresh-cw" class="w-4 h-4"></i>
                            </button>
                            <button type="button" onclick="deleteClassification(<?php echo e($ac->id); ?>)" class="p-2 bg-white text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-xl border border-gray-100 transition-all cursor-pointer relative z-10">
                                <i data-lucide="trash-2" class="w-4 h-4"></i>
                            </button>
                        </div>
                    </div>
...
<?php echo $__env->make('partials._driver_details_modal', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>


<script>
<?php echo $__env->make('partials._driver_details_scripts', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
...
// ─── Constants & State ───
var partsCatalog = <?php echo json_encode($spare_parts ?? [], 15, 512) ?>;
var incidentPartsCart = [];
var incidentServices = [];
var classificationsMap = <?php echo json_encode($classifications->pluck('default_severity', 'name'), 512) ?>;

// Full classification metadata for the smart modal
var classificationsMeta = {};
<?php $__currentLoopData = $classifications; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
classificationsMeta["<?php echo e($c->name); ?>"] = {
    mode: "<?php echo e($c->behavior_mode ?? 'narrative'); ?>",
    subOptions: <?php echo json_encode($c->sub_options ?? $c->getDefaultSubOptions($c->behavior_mode ?? 'narrative'), 15, 512) ?>,
    autoBan: <?php echo e($c->auto_ban_trigger ? 'true' : 'false'); ?>,
    banValue: "<?php echo e(addslashes($c->ban_trigger_value ?? '')); ?>"
};
<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
window.saveQuickPart = async function() {
    ...
    try {
        const res = await fetch("<?php echo e(route('spare-parts.store')); ?>", {
            method: 'POST',
                ...
                <div style="background:#fff;border-radius:20px;max-width:500px;width:100%;box-shadow:0 20px 40px rgba(0,0,0,0.2);overflow:hidden;animation:modal-in 0.2s ease;">
                    <div style="padding:15px 20px;background:#fef2f2;border-bottom:1px solid #fee2e2;display:flex;justify-content:space-between;align-items:center;">
                        <h3 style="margin:0;font-weight:900;color:#991b1b;display:flex;align-items:center;gap:8px;">
                            <i data-lucide="alert-triangle" style="width:20px;height:20px;"></i> SOS Emergency Details
                        </h3>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/u747826271/domains/eurotaxisystem.site/public_html/resources/views/driver-behavior/index.blade.php ENDPATH**/ ?>