        ...
        <div class="bg-white rounded-xl shadow-sm border overflow-hidden">
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y">
                        <?php $__empty_1 = true; $__currentLoopData = $appDrivers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $driver): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <?php
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-full bg-green-100 flex items-center justify-center text-green-700 font-bold text-xs uppercase">
                                        <?php echo e(substr($driver->full_name ?? $driver->name, 0, 1)); ?>

                                    </div>
                                    <span class="font-medium text-gray-900"><?php echo e($driver->full_name ?? $driver->name); ?></span>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-600">
                                <div><?php echo e($driver->email ?? '---'); ?></div>
                                <div class="text-xs text-gray-400"><?php echo e($driver->phone ?? $driver->phone_number ?? '---'); ?></div>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-600 font-mono">
                                <?php echo e($latestBrowser ? $latestBrowser->ip_address : '---'); ?>

                            </td>
                            <td class="px-6 py-4 text-sm text-gray-600">
                                <?php if($latestBrowser): ?>
                                    <div class="truncate max-w-[200px]" title="<?php echo e($latestBrowser->device_info ?? $latestBrowser->user_agent); ?>">
                                        <?php echo e($latestBrowser->device_info ?? $latestBrowser->user_agent ?? 'Unknown Device'); ?>
                            ...
                            <td class="px-6 py-4 text-right">
                                <span class="px-2.5 py-1 rounded-full text-xs font-medium <?php echo e($driver->is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'); ?>">
                            ...
                            <td class="px-6 py-4 text-right relative">
                                <div class="inline-block text-left">
                                    <button type="button" 
                                        onclick="toggleStaffDropdown('app-driver-dropdown-<?php echo e($driver->id); ?>', event)"
                                        class="p-2 hover:bg-gray-100 rounded-full transition-colors focus:outline-none">
                                        <i data-lucide="more-vertical" class="w-4 h-4 text-gray-500"></i>
                                    </button>
                                    <div id="app-driver-dropdown-<?php echo e($driver->id); ?>" 
                                        class="staff-action-dropdown absolute right-6 mt-1 w-32 bg-white border border-gray-100 rounded-xl shadow-xl z-50 hidden animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
                                        <div class="p-1.5 space-y-1">
                                            <form action="<?php echo e(route('staff.destroyAppDriver', $driver->id)); ?>" method="POST" onsubmit="return confirm('Are you sure you want to delete this Driver\'s Mobile App Account? They will lose access to the app immediately.')">
                                                <?php echo csrf_field(); ?>
                                                <?php echo method_field('DELETE'); ?>
                                                <button type="submit" class="w-full flex items-center gap-2 px-3 py-2 text-xs font-bold text-red-600 hover:bg-red-50 rounded-lg transition-all text-left">
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/u747826271/domains/eurotaxisystem.site/public_html/resources/views/staff/index.blade.php ENDPATH**/ ?>