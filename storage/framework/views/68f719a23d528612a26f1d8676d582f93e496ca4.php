...
<div class="bg-white rounded-lg shadow p-6 mb-6">
    <form class="flex flex-col sm:flex-row gap-4" method="GET" action="<?php echo e(route('boundaries.index')); ?>">
        ...
        <div class="flex gap-2">
            <a href="<?php echo e(route('boundary-rules.index')); ?>"
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6" id="fleetStatsBoard">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 flex flex-col justify-center relative overflow-hidden">
        ...
        <div class="flex items-end gap-2 relative z-10">
            <span class="text-4xl font-black text-gray-800 tracking-tighter" id="stat_total_deployable"><?php echo e($fleet_stats['total_deployable']); ?></span>
    ...
    <div onclick="openPlatesModal('remitted')" class="bg-gradient-to-br from-green-500 to-emerald-600 rounded-2xl shadow-lg shadow-green-500/20 p-5 flex flex-col justify-center relative overflow-hidden text-white cursor-pointer hover:scale-[1.02] transition-transform active:scale-95">
        ...
        <div class="flex items-end gap-2 relative z-10">
            <span class="text-4xl font-black tracking-tighter" id="stat_total_remitted"><?php echo e($fleet_stats['total_remitted']); ?></span>
    ...
    <div onclick="openPlatesModal('vacant')" class="bg-gradient-to-br from-red-500 to-rose-600 rounded-2xl shadow-lg shadow-red-500/20 p-5 flex flex-col justify-center relative overflow-hidden text-white cursor-pointer hover:scale-[1.02] transition-transform active:scale-95">
        ...
        <div class="flex items-end gap-2 relative z-10">
            <span class="text-4xl font-black tracking-tighter" id="stat_total_vacant"><?php echo e($fleet_stats['total_vacant']); ?></span>
...
<div id="boundariesTableWrapper">
    <?php echo $__env->make('boundaries.partials._boundaries_table', ['boundaries' => $boundariesArray], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
</div>
...
<div id="boundaryModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm hidden z-50 flex items-center justify-center p-4 transition-all">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[95vh] flex flex-col overflow-hidden">
        ...
        <form id="boundaryForm" method="POST" action="<?php echo e(route('boundaries.store')); ?>" class="flex flex-col flex-1 min-h-0">
            <?php echo csrf_field(); ?>
                            ...
                            <div id="unit_dropdown" class="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-xl max-h-48 overflow-y-auto hidden">
                                <?php $__currentLoopData = $units; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $unit): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <div class="unit-option px-3 py-2 hover:bg-yellow-50 cursor-pointer border-b border-gray-100 last:border-b-0"
                                         ...
                                         data-name="<?php echo e($unit['plate_number']); ?>"
                                         data-plate="<?php echo e($unit['plate_number']); ?>"
                                         data-year="<?php echo e($unit['year'] ?? 0); ?>"
                                         data-model="<?php echo e($unit['make_model'] ?? ''); ?>"
                                         data-rate="<?php echo e($unit['boundary_rate'] ?? 0); ?>"
                                         data-coding-day="<?php echo e($unit['coding_day'] ?? ''); ?>"
                                         data-primary-id="<?php echo e($unit['driver_id']); ?>"
                                         data-secondary-id="<?php echo e($unit['secondary_driver_id']); ?>"
                                         data-expected-id="<?php echo e($unit['current_turn_driver_id']); ?>"
                                         data-deadline="<?php echo e($unit['shift_deadline_at']); ?>"
                                         data-swapped-at="<?php echo e($unit['last_swapping_at']); ?>"
                                         data-has-absent-today="<?php echo e($unit['has_absent_today'] ? 'true' : 'false'); ?>">
                                        <div class="font-black text-sm text-gray-900"><?php echo e($unit['plate_number']); ?></div>
                                        <div class="text-[11px] font-bold text-gray-500"><?php echo e($unit['make_model'] ?? 'N/A'); ?></div>
                                    </div>
                                ...
                                <div class="all-drivers-list">
                                    <?php $__currentLoopData = $all_drivers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $driver): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <div class="driver-option px-3 py-2 hover:bg-yellow-50 cursor-pointer border-b border-gray-100 last:border-b-0"
                                             ...
                                             data-shortage="<?php echo e($driver['net_shortage'] ?? 0); ?>"
                                             data-has-accident-debt="<?php echo e(($driver['has_accident_debt'] ?? 0) > 0 ? 'true' : 'false'); ?>"
                                             data-accident-debt-amount="<?php echo e($driver['total_accident_debt'] ?? 0); ?>">
                                            <div class="font-black text-sm text-gray-900"><?php echo e($driver['name']); ?></div>
                                            <div class="text-[11px] font-bold text-gray-500"><?php echo e($driver['current_plate']); ?></div>
                                        </div>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </div>
                            </div>
...
const boundaryRecords = <?php echo json_encode(collect($boundariesArray)->keyBy('id'), 15, 512) ?>;

// Fleet Stats global object
let currentFleetStats = <?php echo json_encode($fleet_stats, 15, 512) ?>;
...
function performLiveSearch() {
    ...
    searchTimeout = setTimeout(async () => {
        ...
        try {
            const response = await fetch(`<?php echo e(route('boundaries.index')); ?>?${params.toString()}`, {
                headers: {
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/u747826271/domains/eurotaxisystem.site/public_html/resources/views/boundaries/index.blade.php ENDPATH**/ ?>