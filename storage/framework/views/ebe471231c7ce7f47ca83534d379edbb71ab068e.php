<?php $__env->startSection('title', 'Driver Support Chat'); ?>
<?php $__env->startSection('page-heading', 'Driver Support Chat'); ?>
    ...
    <div class="w-80 border-r border-gray-100 flex flex-col bg-gray-50/30">
        ...
        <div class="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
            <?php $__empty_1 = true; $__currentLoopData = $drivers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $driver): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <a href="<?php echo e(route('support.index', ['driver_id' => $driver->id])); ?>" 
                   class="flex items-center gap-3 p-3 rounded-2xl transition-all duration-200 <?php echo e(isset($selectedDriver) && $selectedDriver->id == $driver->id ? 'bg-yellow-600 text-white shadow-lg shadow-yellow-200 translate-x-1' : 'hover:bg-white hover:shadow-md text-gray-700'); ?>">
                    <div class="relative flex-shrink-0">
                        <div class="w-12 h-12 rounded-xl bg-gray-200 flex items-center justify-center font-bold text-lg border-2 border-white shadow-sm overflow-hidden">
                            <?php if($driver->profile_image): ?>
                                <img src="<?php echo e(asset('storage/' . $driver->profile_image)); ?>" class="w-full h-full object-cover">
                            <?php else: ?>
                                <?php echo e(strtoupper(substr($driver->full_name ?? 'D', 0, 1))); ?>

                            <?php endif; ?>
                        </div>
                        <?php if($driver->unread_count > 0): ?>
                            <span class="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white text-[10px] font-black flex items-center justify-center rounded-full border-2 border-white">
                                <?php echo e($driver->unread_count); ?>
                    ...
                    <div class="min-w-0 flex-1">
                        <div class="flex justify-between items-baseline mb-0.5">
                            <h4 class="text-sm font-bold truncate"><?php echo e($driver->full_name); ?></h4>
                            <?php if($driver->latest_message_time): ?>
                                <span class="text-[10px] <?php echo e(isset($selectedDriver) && $selectedDriver->id == $driver->id ? 'text-yellow-100' : 'text-gray-400'); ?>">
                                    <?php echo e(\Carbon\Carbon::parse($driver->latest_message_time)->format('h:i A')); ?>

                                </span>
                            <?php endif; ?>
                        </div>
                        <p class="text-xs truncate <?php echo e(isset($selectedDriver) && $selectedDriver->id == $driver->id ? 'text-yellow-50/80' : 'text-gray-500'); ?>">
                            <?php echo e($driver->latest_message ?? 'No messages yet'); ?>
    ...
    <div class="flex-1 flex flex-col bg-white">
        <?php if($selectedDriver): ?>
            ...
            <div class="p-4 border-b border-gray-100 flex justify-between items-center bg-white/80 backdrop-blur-md z-10">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-full bg-yellow-100 flex items-center justify-center text-yellow-700 font-bold border border-yellow-200">
                         <?php echo e(strtoupper(substr($selectedDriver->full_name ?? 'D', 0, 1))); ?>

                    </div>
                    <div>
                        <h3 class="text-sm font-black text-gray-900"><?php echo e($selectedDriver->full_name); ?></h3>
            ...
            <div id="chatMessages" class="flex-1 overflow-y-auto p-6 space-y-4 bg-gray-50/50 custom-scrollbar">
                <?php $__empty_1 = true; $__currentLoopData = $chatMessages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $msg): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <div class="flex <?php echo e($msg->sender_type == 'admin' ? 'justify-end' : 'justify-start'); ?>" data-msg-id="<?php echo e($msg->id); ?>">
                        <div class="max-w-[70%] group">
                            <div class="flex items-center gap-2 mb-1 <?php echo e($msg->sender_type == 'admin' ? 'flex-row-reverse' : ''); ?>">
                                ...
                                <div class="relative inline-block dropdown-container">
                                    ...
                                    <div class="dropdown-menu absolute right-0 top-full mt-1 hidden bg-white rounded-xl shadow-lg border border-gray-100 py-1 min-w-[170px] z-50">
                                        <button type="button" onclick="openUnsendModal(<?php echo e($msg->id); ?>, this)" class="w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50 flex items-center gap-2">
                            ...
                            <div class="px-4 py-3 rounded-2xl text-sm shadow-sm <?php echo e($msg->sender_type == 'admin' ? 'bg-yellow-600 text-white rounded-tr-none' : 'bg-white text-gray-800 border border-gray-100 rounded-tl-none'); ?>">
                                <?php if($msg->attachment): ?>
                                    <div class="mb-2 rounded-xl overflow-hidden border border-gray-100">
                                        <img src="<?php echo e(asset($msg->attachment)); ?>" class="max-w-full max-h-64 object-cover cursor-pointer" onclick="window.open(this.src, '_blank')">
                                    </div>
                                <?php endif; ?>
                ...
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                    <div class="h-full flex flex-col items-center justify-center text-center p-8">
                        ...
                        <p class="text-sm text-gray-400 mt-1">Send a message to <?php echo e($selectedDriver->first_name); ?> to begin.</p>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <div class="p-4 bg-white border-t border-gray-100">
                <form id="chatForm" action="<?php echo e(route('support.send')); ?>" method="POST" class="flex items-end gap-2">
                    <?php echo csrf_field(); ?>
                    <input type="hidden" name="driver_id" value="<?php echo e($selectedDriver->id); ?>">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        let lastMessageCount = <?php echo json_encode($chatMessages ? count($chatMessages) : 0, 15, 512) ?>;
        const selectedDriverId = <?php echo json_encode($selectedDriver ? $selectedDriver->id : null, 15, 512) ?>;
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/u747826271/domains/eurotaxisystem.site/public_html/resources/views/support/index.blade.php ENDPATH**/ ?>