<?php $__env->startSection('title', 'Daily Financial Ledger - Euro System'); ?>
<?php $__env->startSection('page-heading', 'Daily History & Ledger'); ?>
<?php $__env->startSection('page-subheading', 'Detailed daily tracking of collections, expenses, maintenance, and net yield'); ?>
...
<?php $__env->startSection('content'); ?>
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-slate-200 p-6 mb-8">
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            ...
            <form method="GET" action="<?php echo e(route('analytics.history')); ?>" class="flex flex-col sm:flex-row items-end gap-3 flex-1 lg:max-w-2xl">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm hover:shadow-lg transition-all duration-300 relative overflow-hidden group">
            ...
            <p class="text-2xl font-black text-slate-800"><?php echo e(formatCurrency($totals['revenue'])); ?></p>
        </div>
        ...
        <div class="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm hover:shadow-lg transition-all duration-300 relative overflow-hidden group">
            ...
            <p class="text-2xl font-black text-slate-800 cursor-pointer hover:text-rose-600 transition-colors select-none"
               onclick="openExpensesBreakdown(null, '<?php echo e($date_from); ?>', '<?php echo e($date_to); ?>', 'All Expenses (<?php echo e(formatDate($date_from)); ?> to <?php echo e(formatDate($date_to)); ?>)')">
                <span class="underline decoration-dashed decoration-slate-300 hover:decoration-rose-500"><?php echo e(formatCurrency($totals['expenses'])); ?></span>
            </p>
        </div>
        ...
        <div class="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm hover:shadow-lg transition-all duration-300 relative overflow-hidden group">
            ...
            <p class="text-2xl font-black text-slate-800"><?php echo e(formatCurrency($totals['maintenance'])); ?></p>
        </div>

        
        <div class="p-6 rounded-3xl border shadow-sm hover:shadow-lg transition-all duration-300 relative overflow-hidden group
            <?php echo e($totals['net_profit'] >= 0 ? 'bg-white border-slate-200' : 'bg-rose-50/30 border-rose-100'); ?>">
            <div class="absolute -right-6 -top-6 w-24 h-24 rounded-full opacity-60 group-hover:scale-120 transition-transform
                <?php echo e($totals['net_profit'] >= 0 ? 'bg-indigo-50' : 'bg-rose-100/50'); ?>"></div>
            <div class="relative z-10 flex items-start justify-between mb-3">
                <div class="p-3 rounded-2xl 
                    <?php echo e($totals['net_profit'] >= 0 ? 'bg-indigo-50 text-indigo-600' : 'bg-rose-100 text-rose-600'); ?>">
                    <i data-lucide="banknote" class="w-6 h-6"></i>
                </div>
                <span class="text-[9px] font-black rounded-md uppercase tracking-wider
                    <?php echo e($totals['net_profit'] >= 0 ? 'text-indigo-600 bg-indigo-50' : 'text-rose-600 bg-rose-100'); ?>">Net Yield</span>
            </div>
            <h4 class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Net Yield</h4>
            <p class="text-2xl font-black 
                <?php echo e($totals['net_profit'] >= 0 ? 'text-indigo-700' : 'text-rose-700'); ?>"><?php echo e(formatCurrency($totals['net_profit'])); ?></p>
        </div>
    </div>
                    ...
                    <?php $__empty_1 = true; $__currentLoopData = $ledger; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="text-right py-4 font-bold text-emerald-600">
                                <?php echo e($row['revenue'] > 0 ? '+ ' . formatCurrency($row['revenue']) : '—'); ?>

                                <?php if($row['shortage'] > 0): ?>
                                    <p class="text-[9px] text-rose-500 font-bold mt-0.5">Shortage: -<?php echo e(formatCurrency($row['shortage'])); ?></p>
                                <?php endif; ?>
                            </td>
                            <td class="text-right py-4 font-bold text-rose-500">
                                <?php if($row['expenses'] > 0): ?>
                                    <span class="cursor-pointer hover:text-rose-700 transition-colors select-none underline decoration-dashed decoration-rose-300 hover:decoration-rose-500"
                                          onclick="openExpensesBreakdown('<?php echo e($row['date']); ?>', null, null, 'Expenses Breakdown — <?php echo e(date('F d, Y', strtotime($row['date']))); ?>')">
                                        - <?php echo e(formatCurrency($row['expenses'])); ?>
                            ...
                            <td class="text-right py-4 font-bold text-amber-500">
                                <?php echo e($row['maintenance'] > 0 ? '- ' . formatCurrency($row['maintenance']) : '—'); ?>

                            </td>
                            <td class="text-right px-8 py-4 font-black text-base
                                <?php echo e($row['net_profit'] >= 0 ? 'text-indigo-600' : 'text-rose-600 bg-rose-50/20'); ?>">
                                <?php echo e($row['net_profit'] >= 0 ? '+' : ''); ?><?php echo e(formatCurrency($row['net_profit'])); ?>
    ...
    <script>
        function openExpensesBreakdown(date, dateFrom, dateTo, title) {
            ...
            let url = '<?php echo e(route("analytics.expenses-breakdown")); ?>?';
            if (date) {
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/u747826271/domains/eurotaxisystem.site/public_html/resources/views/analytics/history.blade.php ENDPATH**/ ?>