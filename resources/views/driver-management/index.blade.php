...
<style>
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="flex flex-wrap gap-4">
                                        ...
                                        <div class="px-3 py-1.5 bg-emerald-50 rounded-xl border border-emerald-100">
                                            <span class="text-[9px] font-black text-emerald-400 uppercase block tracking-tighter">Total Paid</span>
    ...
    function openSuspendBanModal(driverId, driverName) {
        document.getElementById('suspendBanDriverId').value = driverId;
        document.getElementById('suspendBanModalSubtitle').textContent = 'Suspend / Ban: ' + driverName;
        document.getElementById('suspendBanReason').value = '';
        document.getElementById('suspendBanReasonCount').textContent = '0 / 500';
        document.getElementById('suspendBanDuration').value = '7';
        document.getElementById('suspendBanActionType').value = 'suspend';
        
        // Show container
        document.getElementById('suspendBanDurationContainer').style.display = 'block';
        
        const modal = document.getElementById('suspendBanModal');
        modal.classList.remove('hidden');
        setTimeout(() => {
            document.getElementById('suspendBanModalContainer').classList.remove('scale-95');
        }, 10);
    }

    function closeSuspendBanModal() {
        document.getElementById('suspendBanModalContainer').classList.add('scale-95');
        setTimeout(() => {
            document.getElementById('suspendBanModal').classList.add('hidden');
        }, 150);
    }

    function toggleDurationField() {
        const type = document.getElementById('suspendBanActionType').value;
        const container = document.getElementById('suspendBanDurationContainer');
        const durationInput = document.getElementById('suspendBanDuration');
    ...
    function submitSuspendBan(event) {
        event.preventDefault();
        
        const driverId = document.getElementById('suspendBanDriverId').value;
        const actionType = document.getElementById('suspendBanActionType').value;
        const durationDays = document.getElementById('suspendBanDuration').value.trim();
        const reason = document.getElementById('suspendBanReason').value.trim();
        ...
        if (actionType === 'suspend') {
            if (!durationDays) {
                alert('Suspension duration is required.');
                return;
            }
            const days = parseInt(durationDays, 10);
            if (isNaN(days) || days < 1 || days > 365) {
                alert('Suspension duration must be a valid number between 1 and 365 days.');
                return;
            }
        }
        ...
        submitBtn.innerHTML = '<span class="inline-block w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></span> Processing...';
        ...
        fetch(`/driver-management/${driverId}/suspend-or-ban`, {
            ...
            body: JSON.stringify({
                ...
                duration_days: actionType === 'suspend' ? durationDays : null,
                reason: reason
            })
        })
        .then(r => r.json())
        ...
        .catch(err => {
            ...
            alert('Failed to apply lockout. Please check inputs and try again.');
            submitBtn.disabled = false;
            submitBtn.innerHTML = origHtml;
        });
    }
