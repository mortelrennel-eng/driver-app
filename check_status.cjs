...
async function runTest() {
    ...
    if (devRes.code === 0 && devRes.result) {
        console.log(`Current UTC Time (Script): ${getTimestamp()}`);
        console.log(`Got ${devRes.result.length} devices.`);
        for (let i = 0; i < Math.min(5, devRes.result.length); i++) {
            const d = devRes.result[i];
            console.log(`- Plate: ${d.deviceName || d.name}, IMEI: ${d.imei}, Status: ${d.status || d.deviceStatus}, GPS Time: ${d.gpsTime}, Heartbeat: ${d.heartTime || '?'}`);
        }
    } else {
        console.log(devRes);
    }
