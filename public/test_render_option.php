...
foreach ($all_drivers as $driver) {
    if (strpos($driver['name'], 'Jesus') !== false) {
        $html = '<div class="driver-option px-3 py-2"
                     data-id="' . htmlspecialchars($driver['id']) . '"
                     data-name="' . htmlspecialchars($driver['name']) . '"
                     data-unit="' . htmlspecialchars($driver['current_unit']) . '"
                     data-plate="' . htmlspecialchars($driver['current_plate']) . '"
                     data-shortage="' . htmlspecialchars($driver['net_shortage'] ?? 0) . '"
                     data-has-accident-debt="' . (($driver['has_accident_debt'] ?? 0) > 0 ? 'true' : 'false') . '"
                     data-accident-debt-amount="' . htmlspecialchars($driver['total_accident_debt'] ?? 0) . '"
                     data-debts="' . htmlspecialchars(json_encode($driver['pending_debts'])) . '">';
        echo "RENDERED HTML:\n" . $html . "\n";
    }
}
